...
- 2026-05-18: v0.12 planning pass added for farther zoom-out, full default framing of tall artworks, and unclipped active timeline selection. Documentation now records the exact `GalleryManager` zoom/fit coupling, the missing artwork-safe viewport deduction, the timeline active-thumb clipping caused by `overflow-y: hidden` + raised transform, and the next acceptance checks for desktop/phone/tablet states.
