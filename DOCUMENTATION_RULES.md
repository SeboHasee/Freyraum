...
- 2026-05-19: v0.14 technical coding plan written. The v0.14 plan.md section was upgraded from a high-level note to a full technical plan with exact code, worked examples, brainstormed options (A/B/C per issue), recommended implementations, and diagnostics additions. Key decisions recorded: lower `MIN_CAMERA_Z` 0.5→0.2 and `MIN_VISIBLE_ARTWORK_FRACTION` 0.28→0.12 together so the fraction guard no longer dominates on large artworks; reduce `INSPECTION_OVERSCROLL` 3.0→1.2; add `PORTRAIT_RESET_EXTRA_Z = 1.5` after the `Math.max()` in `getResetFitZoom()` (margin-based approaches have no effect when `DEFAULT_CAMERA_Z = 7` wins). All markdown files updated.
