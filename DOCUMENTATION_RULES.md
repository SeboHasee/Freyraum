...
- 2026-05-19: importer compatibility fix documented. Added `scripts/run-import-artworks.cjs` launcher and updated both `Update Gallery` launchers to guard Node version before loading ESM importer code. Docs now explicitly require Node.js 18+ and include troubleshooting for `Unexpected token {` from old Node runtimes.
