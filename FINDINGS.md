...
## v0.45 — frame anti-repetition + high-resolution brushed-metal research (2026-05-22, **planned / docs-only**)

### Current observed gap

The v0.44 GLSL frame is a major improvement and the v0.44.1 `tbn` hotfix restores frame visibility, but close inspection can still reveal residual procedural cadence. The requested next step is stricter: **zero visible tiles or repeating patterns**, sharper high-resolution micro-detail when zooming far in, and a slightly rougher / less shiny material.

### Findings

#### Finding 1 — “No visible tiles” requires removing visible coordinate cells, not only removing texture repeats

v0.44 removed repeating DataTextures, which fixed the hard 1-world-unit seam problem. However, the current GLSL still uses anti-tile hash offsets based on coarse cells (`floor(p * freq)`) and a low-frequency blend mask. This is safer than texture repeat, but if the cell domain or blend domain becomes visible on a long flat frame bar, it can still read as repetition.

Next-pass conclusion: use continuous frame-space coordinates plus multiple decorrelated non-periodic domains. Avoid making hash cells the main visible structure.

#### Finding 2 — Object/world-space frame coordinates are safer than relying only on `vUv`

`THREE.ExtrudeGeometry` UVs are generated by `WorldUVGenerator` and historically caused multiple frame artifacts in this repo. A robust zero-visible-tiling pass should inject object-space or world-space position varyings and derive brush coordinates from the frame geometry itself. This reduces dependency on shape UV behavior and avoids accidental UV repeat assumptions.

#### Finding 3 — High-resolution close zoom needs explicit scratch primitives

FBM adds multi-scale grain, but it does not automatically create believable individual scratches. Brushed metal needs sparse, narrow, directional line scratches with varied lengths, widths, intensities, and subtle direction changes. These should influence both normal and roughness so close zoom shows crisp abrasion detail rather than soft noise.

#### Finding 4 — Fine detail must be derivative-aware

Sharper procedural lines can alias during camera movement. Use `fwidth`/derivative-aware line widths or intensity clamps for the scratch layer so close-up detail remains crisp without shimmer, crawling, or moiré.

#### Finding 5 — The current material target is slightly too shiny

PBR metal/roughness guidance places many brushed-metal flat panels around roughness `0.35–0.55`, depending on polish/wear. Current high preset roughness is `0.28`, with clearcoat `0.18`, which can read too glossy/chrome-like. The next pass should raise roughness modestly, reduce clearcoat, and keep anisotropy controlled rather than maximized.

#### Finding 6 — Three.js r166 TBN naming is a hard compatibility constraint

Local verification in `node_modules/three/src/renderers/shaders/ShaderChunk/normal_fragment_begin.glsl.js` shows Three.js r166 builds a local `mat3 tbn`. The injected normal replacement must use `tbn`, not `vTBN`. Documentation that still says `vTBN` is outdated and risks reintroducing the invisible-frame shader compile bug.

### Research references

- Three.js r166 local shader chunks: `normal_fragment_begin.glsl.js`, `normal_fragment_maps.glsl.js`.
- Three.js `Material.onBeforeCompile` shader customization model.
- The Book of Shaders Chapter 13: FBM for layered procedural detail.
- Heitz & Neyret 2018 stochastic/histogram-preserving tiling research: useful anti-repetition concept, but visible cell cadence must be avoided.
- Marmoset / Adobe Substance PBR metal-roughness guidance: calibrate roughness/anisotropy/clearcoat to avoid chrome-like results.

### Planned implementation target

See `plan.md § v0.45` for the full implementation plan. Summary:

1. Add continuous frame-space coordinates.
2. Replace coarse tile jitter with aperiodic multi-domain sampling.
3. Add derivative-aware individual scratch primitives.
4. Improve layered height-to-normal conversion.
5. Increase base roughness and reduce clearcoat slightly.
6. Add diagnostics proving the v0.45 shader path is active.
7. Validate with lint/build plus close-zoom visual QA.

### Status boundary

This is **not shipped runtime behavior yet**. Current runtime remains v0.44.1: visible GLSL frame, no DataTexture repeat seams, but v0.45 strict anti-repetition and sharper-close-detail work remains planned.
