...
// v0.79 — optional customer hub hotspot config. When
// `customer-artworks/hub-hotspots.json` exists and parses as an array, it is
// injected alongside the artwork manifest as `window.__FREYRAUM_HUB_HOTSPOTS`.
// The runtime validates entries and falls back to derived hotspots when the
// file is missing or invalid, so a broken JSON never blocks the gallery.
